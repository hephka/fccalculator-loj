...
- **`index.html`** — Buildings & Research: Warden's Office and 5 of its 6 support buildings (Level 30 → FC10 — Medical Station stops at FC8, not confirmed past that), the FC Lab building itself (Level 1 → 6, gated by Warden's Office), plus each troop's T11 research tree (Shooter/Bomber/Shieldbearer) — gated by a mix of internal prerequisites and FC Lab's own level. Warden's Office and FC Lab sit flat and always visible; the 6 support buildings fold into one collapsible "Support buildings" group, since the prerequisite cascade already works out how far each has to climb. Paliers are offered as a current level only — a target is always a whole FC tier. Resources: FC, AFC, Hyperalloy.
...
- **Also bump it when you correct a cost value, even though nothing about the shape changed.** A saved state carries a full copy of that route's tracks, levels and costs (~79 KB on `index.html`), and `load()` hands that copy straight back — so the file is only the source of truth for a visitor who has no saved state yet. Correct a number without bumping, and everyone who already used that route keeps the old one indefinitely, with no sign anything is stale.
